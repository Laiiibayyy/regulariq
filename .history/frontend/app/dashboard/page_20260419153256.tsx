...
import { getBusiness, downloadAuditPDF } from '@/lib/api';
import ScoreWidget from './ScoreWidget';

const NAV = [
  { label: 'Dashboard', active: true, badge: null, path: '/dashboard' },
  { label: 'Compliance', active: false, badge: 3, path: '/dashboard' },
  { label: 'Documents', active: false, badge: null, path: '/documents' },
  { label: 'Deadlines', active: false, badge: null, path: '/dashboard' },
  { label: 'Alerts', active: false, badge: null, path: '/alerts' },
  { label: 'Employees', active: false, badge: null, path: '/employees' },
];
    ...
    <div className="flex min-h-screen bg-[#060610] relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative z-10">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.05] bg-white/[0.01] backdrop-blur-xl">
          <div>
            <h1 className="text-sm font-bold text-white tracking-tight">Good morning 👋</h1>
            <p className="text-[11px] text-white/30 mt-0.5">Here's your compliance overview</p>
          </div>
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5 hover:border-indigo-500/15 transition-all">
              ...
              <svg width="100%" height="100" viewBox="0 0 300 100" preserveAspectRatio="none">
                ...
                <path d="M0,85 C30,80 50,70 80,60 C110,50 130,52 160,45 C190,38 220,32 250,25 C270,20 285,18 300,15"
                  ...
                  style={{strokeDasharray:1000,strokeDashoffset:1000,animation:'lineGrow 2s ease 0.5s forwards'}}/>
                ...
                <circle cx="300" cy="15" r="8" fill="#6366f1" opacity="0.15"/>
              ...
              <div className="flex items-center gap-5">
                <svg width="90" height="90" viewBox="0 0 100 100" className="flex-shrink-0">
                  ...
                  <circle cx="50" cy="50" r="38" fill="none" stroke="#6366f1" strokeWidth="14"
                    strokeDasharray="162 239" strokeDashoffset="60" strokeLinecap="round" transform="rotate(-90 50 50)"/>
                  <circle cx="50" cy="50" r="38" fill="none" stroke="#4ade80" strokeWidth="14"
                    strokeDasharray="48 239" strokeDashoffset="-102" strokeLinecap="round" transform="rotate(-90 50 50)"/>
                  <circle cx="50" cy="50" r="38" fill="none" stroke="#f87171" strokeWidth="14"
                    strokeDasharray="29 239" strokeDashoffset="-150" strokeLinecap="round" transform="rotate(-90 50 50)"/>
                ...
                {DOCS.map((d, i) => (
                  <div key={i}
                    className={`rounded-xl p-3 text-center border cursor-pointer hover:-translate-y-0.5 transition-all ${
                      d.ok ? 'bg-white/[0.03] border-white/[0.06] hover:border-indigo-500/20' : 'bg-red-500/[0.05] border-red-500/20'
                    }`}>
                    <div className="text-xl mb-1">{d.icon}</div>
                    <p className="text-[10px] text-white/55 font-medium leading-tight">{d.name}</p>
                    <p className={`text-[9px] mt-1 ${d.ok ? 'text-green-400/60' : 'text-red-400/70'}`}>Exp: {d.exp}</p>
                  </div>
                ))}
              </div>
              <button onClick={() => router.push('/documents')}
