...
export default function PricingPage() {
  ...
  const handleCheckout = async (planId: string) => {
    ...
    setLoading(planId);
    try {
      const data = await createCheckout(planId, token);
                  ...
                  <div key={i} className="flex items-center gap-2.5 text-xs text-white/55">
                    <div className="w-4 h-4 rounded-full bg-green-400/15 flex items-center justify-center flex-shrink-0">
                      <svg width="8" height="8" viewBox="0 0 8 8" fill="none">
                        <path d="M1 4l2 2 4-4" stroke="#4ade80" strokeWidth="1.5" strokeLinecap="round"/>
                      </svg>
                    </div>
                    {f}
                  </div>
        ...
        <div className="mt-16 max-w-2xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              ...
              { q: 'What happens after my trial?', a: 'You\'ll be prompted to choose a plan. If you don\'t, your account is paused — not deleted.' },
              { q: 'Can I switch plans later?', a: 'Absolutely. Upgrade or downgrade anytime from your account settings.' },
            ].map((faq, i) => (
              <div key={i} className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-5 py-4">
