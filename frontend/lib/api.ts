...
// Get regulations
export const getRegulations = async (token: string) => {
...
// Get AI summary
export const getAISummary = async (id: string, token: string) => {
  const res = await fetch(`${API_URL}/regulations/${id}/summarize`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${token}` },
  });
  return res.json();
...
export const getAIAdvice = async (
  ...
  const res = await fetch(`${API_URL}/regulations/advice?${params}`, {
    headers: { 'Authorization': `Bearer ${token}` },
  });
  return res.json();
};